	...
	return (
		<SidebarProvider>
			<TooltipProvider>
				...
				{isMobile ? (
					<SidebarInset
						className={"p-2 h-screen space-y-2 " + (native.isNative ? "pb-9 " : "")}
					>
						<div className="w-full h-8 flex flex-row space-x-2">
							...
							<div className="flex-1 size-8 bg-card text-card-foreground rounded-lg ring-1 flex flex-row p-1 space-x-2 ring-sidebar-border items-center">
								{auth.exists ? (
									<>
										{auth.uuid ? (
											<MinecraftAvatar size="size-6" uuid={auth.uuid} />
										) : null}
										<span>{auth.username ? auth.username : auth.name}</span>
									</>
								) : (
									<>Signed Out</>
								)}
